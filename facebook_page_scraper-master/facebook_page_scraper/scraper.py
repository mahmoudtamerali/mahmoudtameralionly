...
class Facebook_scraper:
    ...
    def plsclose(self):
        Utilities._Utilities__close_driver(self.__driver)#test
    ...
    def scrap_to_json(self):
        ...
        if(Finder._Finder__accept_cookies(self.__driver) == -1):#if error, will make error message and return -1
            return -1
        self.__layout = Finder._Finder__detect_ui(self.__driver)#if error, will make error message and return -1
        if(self.__layout == -1):
            return -1
        ...
        if( Utilities._Utilities__wait_for_element_to_appear(self.__driver, self.__layout) == -1):#if error, will make error message and return -1
            return -1
        # scroll down to bottom most
        Utilities._Utilities__scroll_down(self.__driver, self.__layout)
        self.__handle_popup(self.__layout)

        name = Finder._Finder__find_name(self.__driver, self.__layout)  # find name element #test
        if(name == -1):#if error, will make error message and return -1
            return -1
        while len(self.__data_dict) <= self.posts_count:
            self.__handle_popup(self.__layout)
            if(self.__find_elements(name) == -1):
                return -1
        ...
        return json.dumps(self.__data_dict, ensure_ascii=False)#flag edited
    ...
    def scrap_to_csv(self, filename, directory=os.getcwd()):
        try:
            ...
            logger.exception('Error at scrap_to_csv : {}'.format(ex))#needs modification if needed
            return False
    ...
    def __find_elements(self, name):
        ...
        if (all_posts == -1):#if the error is here it will print message then return -1
            return -1
        ...
        for post in all_posts:
            try:
                ...
                logger.warning("Error at find_elements method : {}".format(ex))
                Utilities._Utilities__close_driver(self.__driver)#test
                return -1
